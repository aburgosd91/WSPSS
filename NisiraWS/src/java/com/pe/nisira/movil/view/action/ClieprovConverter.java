 ...
 * @author aburgos
 */
@FacesConverter("clieprovConverter")
public class ClieprovConverter implements Converter {
    
    @Override
    public Object getAsObject(FacesContext fc, UIComponent component, String value) {
        if(value != null && value.trim().length() > 0) {
            try {
                List<Clieprov> lst = (new ClieprovDao()).listarClieprovSerach(Constantes.IDEMPRESAGENERAL,value.trim());
                Clieprov service = lst.isEmpty()?null:lst.get(0);
                return service;
