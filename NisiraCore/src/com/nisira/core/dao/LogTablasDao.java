...
public class LogTablasDao extends BaseDao<LogTablas> {
    public LogTablasDao() {
            super(LogTablas.class);
    }
    public LogTablasDao(boolean usaCnBase) throws NisiraORMException {
            super(LogTablas.class, usaCnBase);
    }

    public LogTablas getPorClavePrimaria(String IDEMPRESA, String IDLOG, String ITEM) throws NisiraORMException {
            List<LogTablas> l = listar("t0.IDEMPRESA = ? and t0.IDLOG = ? and t0.ITEM = ? ", IDEMPRESA, IDLOG, ITEM);
            if (l.isEmpty()) {
                    return null;
            } else {
                    return l.get(0);
    ...
    public List<LogTablas> lstLogTablas(String idempresa,List<Usuario> lstusuario,List<Config_ventana_filtro_web> lstConfig_ventana_filtro_web,
            ...
            ArrayList<LogTablas> lista = new ArrayList<LogTablas>();
            try {
                ResultSet rs = null;
                rs = execProcedure("SP_LOG_TABLAS_DOCUMENTOS",
                        idempresa,
                        xmlNot,
